...
- **Checkpoint logging per business flow**: beyond the mandatory fields above, every flow's code path (controller in, through validation/persistence/outbox, across RabbitMQ, into the consuming service, to completion) is instrumented against a 12-stage checkpoint taxonomy so one `traceId` or `{Flow="..."}` query shows the complete causal chain. See [`checkpoint-logging-standard.md`](checkpoint-logging-standard.md) for the taxonomy, the reference implementation (`kart-identity-service`/`kart-user-service`'s Registration, Login & Authentication flow), and the per-service rollout status.
...
## Database Migrations & Startup Readiness

Concrete Kart policy for every service that owns its own PostgreSQL database via EF Core. Unlike Observability/Error Handling above, this isn't (yet) a shared NuGet package — it's a repeated file-level pattern each service scaffolds for itself, verified against `kart-identity-service` as the reference implementation:

- **Design-time DbContext factory** (`<Entity>DbContextFactory.cs`, `IDesignTimeDbContextFactory<T>`) reading a `<SERVICE>_DB_CONNECTION_STRING` env var, so `dotnet ef` never needs the full Api host's own configuration (JWT keys, GlobalConfig, etc.) just to run a migration.
- **`.config/dotnet-tools.json`** pinning `dotnet-ef` to the same version as that service's own `Microsoft.EntityFrameworkCore.Design` package reference.
- **`scripts/migrate.sh`** (`migrate.sh` / `migrate.sh add <Name>`) + **`.env.example`**, so migrations are one bash command locally, not a memorized `dotnet ef` invocation.
- **`Dockerfile.migrate`** — a self-contained EF migrations-bundle image, entrypoint reading the same `<SERVICE>_DB_CONNECTION_STRING` env var, smoke-built by a `migration-bundle` CI job (`docker build -f Dockerfile.migrate ...`) so a broken migration never reaches deploy unnoticed.
- **`<Entity>DbHealthCheck`** wired to `/health/ready` (tagged `"ready"`) that fails readiness on *any* pending migration, not just an unreachable database — a connectable-but-unmigrated Postgres must not receive traffic. `/health/live` stays dependency-free (no DB check).
- **`StartupConnectivityChecks`** — one Connecting/connected log pair per infra dependency (Postgres/Redis/RabbitMQ) right after `app.Build()`, so a misconfigured dependency fails loud at boot instead of on the first request. Must no-op under `app.Environment.IsEnvironment("Testing")` — every `WebApplicationFactory`-based test factory that doesn't already swap out the real infra registrations needs `builder.UseEnvironment("Testing")` for exactly this reason.

**Rollout status** (as of 2026-07-29): `kart-identity-service` and `kart-category-service` have the full pattern. `kart-cart-service`, `kart-inventory-service`, `kart-offer-service`, `kart-payment-service`, `kart-product-service`, and `kart-user-service` each already have the DbContext factory but are missing everything else above. `kart-delivery-tracking-service` and `kart-search-service` are scaffolded but don't own a PostgreSQL database (different data store) — not applicable. Every other service repo isn't scaffolded yet. See [`service-infra-rollout-checklist.md`](service-infra-rollout-checklist.md) for the step-by-step prompt used to bring a service to parity, and update that file's status table as each service is done.
