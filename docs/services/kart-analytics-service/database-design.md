...
**Addition note (this pass — `product_performance_dashboard`, 11th read model):** `requirement-spec.md` §6 item 7, `ddd-model.md`'s Modeling Decision 6 (and its `product_performance_dashboard` entry under "Read Models"), and `edge-cases.md`'s two new "product-performance" entries ("Ranking Ties at the Limit Cutoff" and "Rank Order/Membership Changes on Reconciliation") add one new MongoDB read-model collection, additive to the ten already built below — driven by the new downstream consumer `kart-ai-assistant-service` ([ADR-0024](../../adr/0024-ai-assistant-service-scope-and-integration.md)) per `docs/requirements/genai-business-assistant-spec.md` §10.3. This is a **read-model-only** addition: no new PostgreSQL table, no new column on any of the four existing write-model tables, and no new ingestion path. The new projector below reads `OrderCreated`'s `items` array out of the already-ingested `analytics_raw_events.payload` — the same raw store every existing projector already reads from — via the exact same two-consumer (near-real-time incremental + nightly batch reconciler) pattern documented for the other ten collections; nothing about the write model changes. **Scope note, so this isn't conflated with a different, unrelated open item:** [ADR-0027](../../adr/0027-order-confirmed-address-shape-gap.md) flags a producer-side contract gap for `OrderConfirmed.address` (blocking a *different*, not-yet-built future capability — geographic analytics) — it does not touch `OrderCreated.items`, whose `{sku, qty, unitPrice: {amount, currency}}` shape genai-business-assistant-spec.md §10.3 confirms directly against `kart-order-service`'s own approved `database-design.md`/`ddd-model.md`. This addition has no dependency on ADR-0027's open item and introduces no new producer-side contract risk.
...
| `product_performance_dashboard.sku` / `.category` (MongoDB read model, **added this pass**) | Any caller holding the `analytics.dashboards.read` scope (internal-only; §24.1.2 above) | Every caller outside that scope (no public Gateway route at all) | Product-catalog identifiers, not customer/end-user data — the same no-PII treatment every other dashboard's `sku`/`category` field already gets (e.g. `revenue_dashboard`, `inventory_movement_dashboard`), extended unchanged to this 11th collection. `revenue`/`unitsSold`/`orderCount` are pre-aggregated sums/counts, not personal data, same as every other dashboard's aggregate fields |

**Why this list and no more:** every other column across all four PostgreSQL tables and all eleven MongoDB dashboard/funnel collections (including `product_performance_dashboard`, added this pass) is either an opaque identifier (`event_id`, `dlq_id`, `run_id`, `redaction_id`), operational metadata (`schema_id`, `retry_count`, `status`), a product-catalog identifier (`sku`, `category` — product data, not personal data about an identifiable individual), or a pre-aggregated count/sum/bucket (`revenue`, `orderCount`, `unitsSold`, `ratingDistribution`) — none of it is personal data about an identifiable individual. `analytics_pii_redactions.user_id` itself is not classified as a masked column here because, per the Row-Level Security Policy section above, no endpoint exposes this table to any caller at all — there is no response DTO for a masking rule to apply to yet.
...
One collection per dashboard/funnel already enumerated in requirement-spec.md §6 D4a and exposed as an endpoint in `api-contract.yaml` — **eleven, as of this pass** (ten original plus `product_performance_dashboard`, added this pass; see "Addition note" above). Every collection is rebuilt from `analytics_raw_events` by exactly two projection consumers, never written to by the query API directly (`ddd-cqrs-standards.md`):
...
// Added this pass (11th collection; requirement-spec.md §6 item 7; ddd-model.md Modeling Decision 6;
// genai-business-assistant-spec.md §10.3). One doc per (granularity, bucketStart, sku) — always
// carries a sku (unlike revenue_dashboard's optional sku?), since the ranking query needs one row
// per candidate product. Backs GET /internal/v1/dashboards/product-performance.
db.createCollection("product_performance_dashboard")
// { _id, granularity, bucketStart, sku, category, revenue: {amount, currency}, unitsSold, orderCount, generatedAt, isProvisional, reconciledThrough }
```

**`product_performance_dashboard`'s projector (added this pass)** follows the identical two-consumer pattern above, with no new consumer type and no new ingestion path:

1. **Near-real-time incremental projector** — for each newly-landed `OrderCreated` row in `analytics_raw_events`, reads the row's already-ingested `payload.items` array (confirmed shape, `{sku, qty, unitPrice: {amount, currency}}` per line item — genai-business-assistant-spec.md §10.3, checked directly against `kart-order-service/database-design.md`'s/`ddd-model.md`'s own `OrderLineItem`/`order_items` definition), and for each line item upserts/increments the provisional `(granularity, bucketStart, sku)` document's `revenue`, `unitsSold`, and `orderCount` — the same "read `payload` fields for projection purposes only, never decompose them onto `IngestedEvent` itself" treatment `revenue_dashboard`'s own projector already applies to `OrderCreated`/`PaymentCompleted` totals "by SKU/category" (requirement-spec.md §6 D4a; `ddd-model.md`'s Modeling Decision 3).
2. **Nightly batch reconciler** — recomputes every `(granularity, bucketStart, sku)` bucket from `analytics_raw_events` in full (re-scanning every `OrderCreated` row's `payload.items` for the bucket, never incrementally), overwrites the provisional document, sets `isProvisional: false` and `reconciledThrough`, exactly as for the other ten collections. Because this is the first dashboard whose serving query truncates a ranked list (`$sort` + `$limit`), reconciliation here can additionally change which SKUs even appear in a top/bottom-N response, not merely correct a value in place — ddd-model.md's and edge-cases.md's "Rank Order/Membership Changes on Reconciliation" entry (added this pass) already covers this; no new signal beyond the standard `isProvisional`/`reconciledThrough` envelope fields is introduced to disclose it.

**No new/unconfirmed producer-side contract dependency:** this projector's only source field is `OrderCreated.items`, whose shape is already confirmed (genai-business-assistant-spec.md §10.3, checked against `kart-order-service`'s own approved docs) — it does not touch `OrderConfirmed.address`, the field [ADR-0027](../../adr/0027-order-confirmed-address-shape-gap.md) flags as an unconfirmed producer-side gap for a separate, not-yet-built geographic-analytics capability. Nothing in this addition is gated on ADR-0027's open item.
...
| `idx_analytics_raw_events_type_occurred` | Every projector's "events of type X since the last watermark, in event-time order" scan | Both the near-real-time incremental projector and the nightly reconciler run this exact pattern for every one of D4a's eleven dashboards/funnels (ten original plus `product_performance_dashboard`, added this pass) — without it, each projection run degrades to a full scan of an indefinitely-retained table |
...
| Mongo (**added this pass**): `{granularity:1, bucketStart:1}` on `product_performance_dashboard` | Same `From`/`To`/`Granularity` bucket-scoping query as every other dashboard | Same reasoning as the shared `{granularity:1, bucketStart:1}` row above — this collection still supports the plain, non-ranked bucket lookup, not only the ranking query below |
| Mongo (**added this pass**): six compound indexes on `product_performance_dashboard` — `{granularity:1, bucketStart:1, category:1, <metric>:-1, sku:1}` and `{granularity:1, bucketStart:1, <metric>:-1, sku:1}`, one pair per rankable `<metric>` ∈ `{"revenue.amount", "unitsSold", "orderCount"}` | `GET /internal/v1/dashboards/product-performance?from=&to=&granularity=&metric=&category=&limit=&direction=` — the `$sort` on the requested `metric` (with `sku` ascending appended as the deterministic tiebreak) + `$limit`, both with and without the optional `category` filter | Direct support for edge-cases.md's "Ranking Ties at the Limit Cutoff" decision: `sku` stored ascending after each metric so a tie at the `limit` cutoff resolves identically on every repeat call, never left to Mongo's unspecified order for equal sort-key values (ddd-model.md's "ranking determinism" domain-invariant note). A separate index per `metric` is required because the endpoint's `metric` parameter selects one of three physically distinct sort orders — no single B-tree can serve all three as an index-only sort. The `category`-prefixed variant serves the filtered case as a pure index scan (no in-memory sort); the `category`-omitted variant serves the "all categories" case off the `(granularity, bucketStart)` prefix alone, at the cost of an in-memory sort/tiebreak over that bucket's candidate SKUs — accepted because a single bucket's SKU cardinality is small relative to the raw event store (see Partitioning/Sharding below), not a full collection scan |
| Mongo (**added this pass**): index direction and `direction=asc` (bottom-N) queries on `product_performance_dashboard` | The endpoint's `direction=desc\|asc` parameter | Each index above is stored `<metric>:-1` (descending), matching `direction=desc`/top-N — cited as this endpoint's dominant real-world use case (genai-business-assistant-spec.md's own founding example, "top 5 selling products"; the business flows catalog's "top N products" pattern) — as a pure forward index scan. `direction=asc`/bottom-N reuses the same index for its `(granularity, bucketStart[, category])` scoping but requires an in-memory re-sort to preserve the same "`sku` ascending" tiebreak in the opposite metric direction (a mirrored `<metric>:1, sku:1` index pair would remove this, at the cost of doubling the index count); accepted as a bounded, cheap cost for the same reason as the row above — revisit only if `direction=asc` query volume or a bucket's SKU cardinality later proves this a bottleneck, the same "revisit only if evidence shows it" qualifier this document already applies to PostgreSQL sharding below |
...
- **MongoDB collections: single replica set, no sharding, at current scale.** Unlike the raw event store, these collections are bounded by **time-bucket cardinality × dimension cardinality** (e.g. day-buckets × SKU count), not raw event volume — several orders of magnitude smaller than `analytics_raw_events`. If a specific collection's dimensional cardinality (e.g. `revenue_dashboard` by SKU) later proves large enough to threaten the P95 < 2s budget, shard that collection on `{granularity, bucketStart}` (matching its own primary query pattern above) rather than sharding the whole read model preemptively. **`product_performance_dashboard` (added this pass) introduces no new sharding/partitioning need and is not an exception to this reasoning** — it has the exact same `(granularity, bucketStart) × sku` bound as `revenue_dashboard`'s own dimensional shape (one row per candidate SKU per bucket, not per raw event), so the same "revisit only if dimensional cardinality later threatens the latency budget" qualifier applies to it unchanged, with no distinct justification needed.
...
**Addendum — this pass's addition (`product_performance_dashboard`, 11th read model, added this pass).** Per `database-design-agent.md`'s Human Approval Required clause, "read-model/projection changes can be marked approved directly" — unlike the write-model schema above, no human approval gate applies to this addition, since it introduces no new PostgreSQL table/column and no new ingestion path (see "Addition note" above). Marked `[x] Approved (read-model addition)` on that basis.
- [x] Approved (read-model addition — `product_performance_dashboard`)
