...
## kart-ai-assistant-service

See [full architecture doc](../services/kart-ai-assistant-service/architecture.md). A new, independently deployed bounded context — the platform's 19th deployable repo, [ADR-0024](../adr/0024-ai-assistant-service-scope-and-integration.md) — not a gap-fill or extraction from any existing service.

| Direction | Peer | Mechanism | Type |
|---|---|---|---|
| Inbound (client) | Admin / Support Agent, via `kart-admin-web` → API Gateway | `POST /v1/ai-assistant/query` (`security: bearerAuth: [ai-assistant.query]`, [ADR-0025](../adr/0025-ai-assistant-query-scope.md)) | Sync |
...
| Outbound (external, non-Kart) | Model Gateway / LLM Provider | Structured-output "plan" call, grounded "explain" call, via the platform's `ModelProvider` interface | Sync (external) |
| Outbound (published) | — none — | This service publishes zero platform events | — |
| Inbound (consumed) | — none — | This service consumes zero platform events | — |

No Saga participation — not a step in the Order Saga's success or compensation flow (BRD §12), and it holds no aggregate any saga step would need to compensate. Exactly **one** synchronous dependency on another Kart bounded context (`kart-analytics-service`), no other Kart-service dependency of any kind, and zero asynchronous publish/consume relationships — all three closed decisions restated, not re-derived, from [ADR-0024](../adr/0024-ai-assistant-service-scope-and-integration.md). No sync/async distributed-monolith risk is flagged: the one synchronous edge is non-chatty (one call per user-facing turn), non-transitive (`kart-analytics-service`'s own `architecture.md` confirms it has zero synchronous outbound dependencies of its own, so the call terminates there rather than chaining further), and circuit-breaker-protected with a fail-closed posture (design-decisions.md's Resilience Pattern decision) rather than silently cascading. The Model Gateway/LLM-provider edge is an external-system dependency, not a Kart bounded-context coupling, and is mitigated the same way (independent circuit breaker + bounded retry). See [ADR-0024](../adr/0024-ai-assistant-service-scope-and-integration.md), [ADR-0025](../adr/0025-ai-assistant-query-scope.md), [ADR-0026](../adr/0026-seller-vendor-scope-gap-ruling.md), and [ADR-0027](../adr/0027-order-confirmed-address-shape-gap.md).

## kart-shopping-assistant-service

See [full architecture doc](../services/kart-shopping-assistant-service/architecture.md). A new, independently deployed, **mutation-capable** bounded context — the platform's **20th** deployable repo, [ADR-0028](../adr/0028-shopping-assistant-service-scope-and-integration.md) — not a gap-fill or extraction from any existing service, and not a reopening of ADR-0024's closed ruling for its read-only sibling.

| Direction | Peer | Mechanism | Type |
|---|---|---|---|
| Inbound (client) | Customer (authenticated) or Guest, via `kart-web` → API Gateway | `POST /v1/shopping-assistant/query` (`security: bearerAuth: [shopping-assistant.act]` or anonymous, [ADR-0029](../adr/0029-shopping-assistant-scope-and-guest-access.md) — Tier 1 is a role-*exclusion*, not a role-requirement) | Sync |
| Outbound | `kart-order-service` | `GET /orders/{id}`, `POST /orders/{id}/cancel`, `POST /orders` (checkout-create) | Sync, per-edge circuit breaker (500ms timeout on read/cancel; 2.5s on checkout-create, which itself chains into Order's own saga-step-1 Inventory-reserve call, ADR-0009) |
| Outbound | `kart-cart-service` | `GET /cart`, `POST /cart/items`, `POST /cart/checkout` | Sync, per-edge circuit breaker |
| Outbound | `kart-offer-service` | `POST /coupons/validate`, `GET /promotions/active` | Sync, per-edge circuit breaker |
| Outbound | `kart-search-service` | `GET /search` | Sync, per-edge circuit breaker |
| Outbound | `kart-product-service` | `GET /products/{sku}` | Sync, per-edge circuit breaker |
| Outbound | `kart-recommendation-service` | `GET /recommendations/{userId}` | Sync, per-edge circuit breaker |
| Outbound | User Service | `GET /users/{id}` | Sync, per-edge circuit breaker |
| Outbound | `kart-delivery-tracking-service` | `GET /tracking/{trackingId}` | Sync, per-edge circuit breaker |
| Outbound | `kart-wishlist-service` | `/wishlist` (add / move-to-cart) | Sync, per-edge circuit breaker |
| Outbound (external, non-Kart) | Model Gateway / LLM Provider | Plan → confirm → execute → summarize calls, capability-tier request, no vendor named | Sync (external) |
| Outbound (published) | — none — | Zero platform events published for v1 (§9 item 4, mirroring ADR-0024) | — |
| Inbound (consumed) | — none — | Zero platform events consumed for v1 | — |

Nine synchronous outbound dependencies — the platform's widest fan-out placed so far, wider than `kart-admin-service`'s own five. **No edge of any kind to `kart-payment-service`** — deliberately and permanently excluded (ADR-0028); the only path by which this service can affect money movement is indirectly, via `kart-order-service`'s own cancel/return-request surface. Each of the nine edges is independently circuit-breakered (`purgatory`, Redis-backed shared state across replicas, design-decisions.md), so no single downstream outage degrades more than the specific intents that peer backs (FR-003's closed intent→endpoint registry structurally prevents any one conversational turn from fanning out across more than the one or two peers its resolved intent actually needs). Two already-existing, inherited two-hop synchronous chains are named, not newly introduced, by this service's own `architecture.md`: (1) the checkout-create call into Order's own already-approved Inventory-reserve saga leg (2s budget, ADR-0009), exercised by every existing checkout entry point on the platform, and (2) Cart's/Recommendation's own already-approved fail-open lazy-validation calls into Product/Inventory. Both terminate at the second hop (`kart-inventory-service`'s and `kart-product-service`'s own entries above confirm zero further synchronous outbound dependency of their own) — no distributed-monolith risk beyond these two already-accepted, unmodified patterns is introduced. Not a participant in the Order Saga (BRD §12) — secondary availability tier (99.9%, requirement-spec §9 item 1). See [ADR-0028](../adr/0028-shopping-assistant-service-scope-and-integration.md), [ADR-0029](../adr/0029-shopping-assistant-scope-and-guest-access.md), and [ADR-0030](../adr/0030-shopping-assistant-python-stack-exception.md).

**Pre-existing gap discovered during this pass (not addressed — out of scope for a `kart-shopping-assistant-service`-targeted run):** `kart-search-service` and `kart-user-service` each have their own approved `architecture.md` with a fully resolved dependency graph, but neither's own Architecture Agent pass ever appended a corresponding entry into this file or a node into `container-diagram.md`, despite that diagram's prior caption claiming both were already placed. This document adds only the minimal node needed in `container-diagram.md` to represent this service's own new edges to them accurately (see that file's caption); backfilling either service's own full `## kart-search-service` / `## kart-user-service` section here is their own service's Architecture Agent output to produce, not this run's.
