...
### 3.8 Shopping Assistant (`Customer` role — includes anonymous/guest access for a subset of intents)

New feature area, additive to §3.1–§3.7 above; it does not renumber or restructure any of them. Sourced from a different, newly-approved backend — `kart-shopping-assistant-service`, a new, independently-deployed bounded context (ADR-0028) — not from any of the eighteen backend services §3.1–§3.6 already consume directly. This is the structural counterpart, on the `kart-web` side, to `kart-admin-web`'s own §3.6 AI Assistant addition — same additive pattern, deliberately different backend, role, and mutation posture (see boundary rationale in `kart-shopping-assistant-service/requirement-spec.md` §1 and ADR-0028).

- **Mixed anonymous/authenticated access, unlike `kart-admin-web`'s AI Assistant.** A guest session may use the assistant for read-only intents (product discovery/search, comparison, general Q&A) without signing in — matching this app's own existing anonymous-browsing posture for Catalog (§3.1) — but every mutating intent (cancel, return, coupon-apply, checkout, cart-add) requires the same authenticated `Customer` session this app already establishes for §3.2–§3.5 (`docs/services/kart-shopping-assistant-service/requirement-spec.md` §1.2; ADR-0029's Tier-1 role-*exclusion*-not-role-*requirement* Gateway posture).
- **Chat panel interaction, action-capable.** A single free-text conversational interface — one chat panel per conversation session — through which a `Customer` can track an order, cancel an order, request a return, discover/compare products, apply a coupon, add an item to cart, or check out, in natural language (`kart-shopping-assistant-service/requirement-spec.md` §2's intent catalog). Unlike `kart-admin-web`'s read-only AI Assistant, **every mutating action here is confirmed in plain language, naming the concrete effect, before it executes** — the chat UI must render a distinct confirmation-required turn (not just an answer/table) and require an explicit affirmative before submitting it back (`requirement-spec.md` FR-004; `kart-shopping-assistant-service/edge-cases.md`'s own confirm-then-execute race analysis, which this UI must respect by re-confirming rather than assuming a stale confirmation still holds).
- **Backend.** Calls the new `kart-shopping-assistant-service` (ADR-0028) exclusively via `kart-api-gateway` — same "never bypass the gateway" rule as every other backend call this app makes (`architecture.md`). Request shape: `POST /v1/shopping-assistant/query` (`kart-shopping-assistant-service/api-contract.yaml`).
- **Access gating.** For an authenticated session, reuses the existing `Customer` JWT already issued by `kart-identity-service` for this app (§3.5) — no new login path — additionally scoped by the new `shopping-assistant.act` scope (ADR-0029), embedded in that JWT via Identity's existing role→scope mapping. For a guest session, no JWT is required for the read-only intents named above; a control for a mutating intent renders as a sign-in prompt (not hidden — the assistant should say what it can't do and why) for an unauthenticated caller, matching the "UX convenience, not the enforcement point" rule this app already applies elsewhere (§5) — the authoritative gate is `kart-shopping-assistant-service`'s own boundary check (ADR-0029), not this client.
- **Confirmed capability gaps, surfaced honestly, not hidden.** Per `kart-shopping-assistant-service/requirement-spec.md` §2/§10, three of the assistant's intents currently resolve to an explicit "not supported yet" response rather than a working action: a direct "just refund me" request outside cancel/return, coupon auto-discovery (no backing endpoint exists yet on `kart-offer-service`), and "wrong item received → replacement" (no Exchange bounded context exists anywhere on the platform). The chat UI must render these as the assistant's own honest `unsupported` response type (`kart-shopping-assistant-service/api-contract.yaml`'s `responseType` enum), never as a generic error — this is a resolved (§10 Business-2 is genuinely open at the platform level, but the *client's own rendering behavior* for an `unsupported` response is not) requirement, not an open question for this document.
- **Cross-customer safety.** No confirmation step or conversational framing in this feature can act on another customer's order/cart/address — enforced entirely server-side (`kart-shopping-assistant-service/requirement-spec.md` §5's domain invariant); this client renders whatever the backend actually resolves and never locally assumes ownership of a resource id mentioned in conversation.
...
6. **Shopping Assistant (§3.8) — added additively, same maintenance policy as resolution #3 above.** A new feature area sourced from a newly-approved backend outside the eighteen services §3.1–§3.6 already consume, following the identical pattern `kart-admin-web/requirement-spec.md` §3.6 already established for its own AI Assistant addition — new backend, new scope, no renumbering of existing sections. Unlike that precedent, this feature mutates real orders/carts/money and has a genuine anonymous-caller population, so its access-gating story (§3.8) is materially different, not a copy-paste of the admin console's read-only pattern.
